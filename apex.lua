    ...
    task.spawn(function()
        while flyEnabled do
            ...
            if hum.MoveDirection.Magnitude > 0 then
                -- Di mobile, gerakin joystick sambil arahin kamera ke atas/bawah
                direction = camera.CFrame:VectorToWorldSpace(Vector3.new(hum.MoveDirection.X, 0, hum.MoveDirection.Z))
                -- Modifikasi arah supaya ngikutin pandangan vertikal kamera
                direction = camera.CFrame.LookVector * (hum.MoveDirection.Z < 0 and -1 or (hum.MoveDirection.Z > 0 and 1 or 0)) +
